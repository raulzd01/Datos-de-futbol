...
def update_seasons_index(code, name, seasons):
    """
    Guarda qué temporadas hay disponibles para esta liga, para que la
    web pueda pintar un selector de temporada. Solo incluye temporadas
    desde 2020 en adelante para no llenar el selector de años muy viejos.
    """
    years = sorted({s["id"] for s in seasons if s["id"] >= 2020}, reverse=True)
    return years
...
if __name__ == "__main__":
    ...
    for code, name in COMPETITIONS.items():
        ...
        all_seasons = get_available_seasons(code)
        seasons_by_competition[code] = update_seasons_index(code, name, all_seasons)
